...
- **`TextMetrics.actualBoundingBoxAscent` is quantized to whole pixels in Chromium, so it cannot
  ...
  used; prefer it.
- **A wall-clock budget calibrated on a developer machine can have no margin at all on CI, while the regression it guards is an order of magnitude.** `interner.test.ts` asserted one run of 1,000,000 interns at `< 500 ms`: **83 ms** on a developer machine, so it read as 6× headroom — and **509 ms** on a contended `windows-latest` runner, where the same code is simply ~6× slower. It passed on the commit before and failed on the commit after, neither of which touched the intern hot path. **`R183-ci-flakes.md` §6 forbids raising such a ceiling without first measuring**, precisely because raising it is the move that hides a real regression; the measurement here was 82.7 ms before the change and 83.4 ms after, on one machine. §6's own first branch then applies: *the shape is wrong for CI, not the constant.* **The repair is to assert the algorithmic property, which no machine's speed can move** — interning is a hash lookup, so growing the distinct-name count 100× must not grow the per-occurrence cost. Measured 1.48×; mutating the hash so every name collides into one bucket takes it to **89.3×**, against a 5× bound. Both halves run back to back in the same test, so contention lands on both. Keep an absolute ceiling only as a catastrophe net, set from what a regression looks like rather than from what a clean run measures.
